...
#actual main network class for learning
class Network(object):
    ...
    def feedforward(self, a):
        for bias, weight in zip(self.biases, self.weights):
            a = sigmoidFunction(np.dot(weight, a) + bias)
        return a
    ...
    def update_mini_batch(self, mini_batch, learningRate):
        ...
        self.weights = [
            w-(learningRate / len(mini_batch)) * nw
            for w, nw in zip(self.weights, nabla_w)
            ]
        
        self.biases = [
            b - (learningRate / len(mini_batch) * nb)
    ...
    def backprop(self, x, y):
        ...
        for bias, weight in zip(self.biases, self.weights):
            ...
            delta = self.cost_derivatives(activations[-1], y) * sigmoidFunction(zs[-1])
            nabla_b[-1] = delta
